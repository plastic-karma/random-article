...
do
  ...
  last_modified_date=$(curl -s "https://api.nytimes.com/svc/archive/v1/${api_year}/${api_month}.json?api-key=$NYT_KEY" -I | grep "Last-Modified" | sed "s/Last-Modified: //g" | sed "s/[ ,:]/_/g")
